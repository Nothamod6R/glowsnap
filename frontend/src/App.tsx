...
import { ResizeToPalette, ResizeToStudio } from '../wailsjs/go/main/App'; 
    ...
    <div className="w-full wails-drag h-full flex items-center justify-center bg-transparent select-none overflow-hidden">
      <AnimatePresence mode="wait">
        ...
        {mode === 'palette' && (
          ...
          >
            <ToolButton icon={<Camera size={18} />} label="Full Screen" />
        ...
        {mode === 'studio' && (
          ...
          >
            <header className="flex items-center justify-between px-6 py-4 border-b 0">
              <div className="flex items-center gap-3">
                <motion.button
                  whileHover={{ scale: 1.05, x: -2 }}
                  whileTap={{ scale: 0.95 }}
                  onClick={switchToPalette}
                  className="flex items-center gap-2 px-3 py-1.5 rounded-xl b hover:bg-white/15 text-xs border border-white/10 text-white/80 hover:text-white"
                >
                  <ArrowLeft size={14} />
                  <span>Palette</span>
                </motion.button>
