...
export function SectionHeader({ title, description }: { title: string; description?: string }) {
  return (
    <div className="px-1 pt-1 pb-2">
      ...
      {description && <div className="text-[11px] text-white/40 leading-4 mt-0.5">{description}</div>}
    </div>
  );
}
...
export function NumberInput({ value, onChange, min, max, step = 1, suffix, disabled }: NumberProps) {
  return (
...
export function PathPicker({ value, onPick, title }: PathProps) {
  return (
    <div className="flex items-center gap-1.5 min-w-0">
      <span className="text-[11px] text-white/50 font-mono truncate max-w-[180px]" title={value}>
        {value}
      </span>
      <button
        type="button"
...
export function Stepper({ value, onChange, min = 0, max = 60 }: { value: number; onChange: (v: number) => void; min?: number; max?: number }) {
